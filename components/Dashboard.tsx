...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const animalStats = useMemo(() => {
      const catAnimals = animals.filter((a: Animal) => a.category === activeTab && !a.archived);
      ...
      for (const animal of catAnimals) {
          ...
          for (const l of logs) {
              if (l.log_type === LogType.WEIGHT) weights.push(l);
              else if (l.log_type === LogType.FEED) feeds.push(l);
          }

          weights.sort((a: LogEntry, b: LogEntry) => new Date(b.log_date).getTime() - new Date(a.log_date).getTime());
          const todayWeight = weights.find((l: LogEntry) => l.log_date.toISOString().startsWith(viewDate));
          const todayFeed = feeds.find((l: LogEntry) => l.log_date.toISOString().startsWith(viewDate));
          ...
          animalData.set(animal.id, { 
              ...
              todayFeed, 
              previousWeight: weights.find((l: LogEntry) => !l.log_date.toISOString().startsWith(viewDate))
          });
      }
  ...
  const filteredAnimals = useMemo(() => {
    let result = animals
      .filter((a: Animal) => a.category === activeTab && !a.archived && a.name.toLowerCase().includes(searchTerm.toLowerCase()));
